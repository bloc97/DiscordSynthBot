 ...
 * @author bowen
 */
public class MidiEventListener4 extends ListenerAdapter {
        ...
        for (String schar : chars) {
            ...
            if (c == '_') {
                ...
                if (isInTimeSignature) {
                    Map.Entry<Float, Float> signature = getTimeSignature(timeSignature);
                ...
                if (isInKeySignature) {
                    ...
                    int dt = getDt(multiplier, periodTimeMultiplier, fractionalMultiplier, beatTimeStep);
                ...
                multiplier = (float)Math.pow(2, realDurationMultiplierPow) * (denominator / 4f);
    ...
    public static int getDt(float multiplier, float periodTimeMultiplier, String fractionalMultiplier, int beatTimeStep) {
        ...
        return Math.max(Math.round(beatTimeStep * realMultiplier * periodTimeMultiplier), 2); //dt has to be at least 2

    }
    
    public static Map.Entry<Float, Float> getTimeSignature(String timeSignature) {
    ...
    public static MidiSendHandler getGuildHandler(Guild guild) {
        ...
        try {
            ...
            Logger.getLogger(MidiEventListener4.class.getName()).log(Level.SEVERE, null, ex);
